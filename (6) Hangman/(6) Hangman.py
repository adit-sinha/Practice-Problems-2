...
def man(wrg):
    ...
    wordleftlist = []
    #prints board
    #leftlist contains letters left to be used by user
    for term in usedlist:
          if len(term) == 1:
            if term in leftlist:
              leftlist.remove(term)
    for aterm in word:
      if aterm not in usedlist:
        wordleftlist.append(aterm)
    print('''|--!
|  O''')
    if wrg <=2:
        if wrg == 0:
        ...
        print('|* | *')
        print('| / \ ')
        conflist = [random.choice(leftlist), random.choice(leftlist), random.choice(leftlist), random.choice(wordleftlist)]
        #conflist contains letters to confuse user
        #ordlist rearranges contents of the conflist to provide the user with a random assortment of letters
        ordlist = []
        for i in range(4):
          randterm = random.choice(conflist)
          ordlist.append(randterm)
          conflist.remove(randterm)
        print(f'One of the letters may be one of {ordlist[0]},{ordlist[1]},{ordlist[2]} or {ordlist[3]}')
    elif wrg == 5:
        print('| /|\ ')
        print('|* | *')
        print('|_/ \_')
